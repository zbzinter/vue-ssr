...
import { Dropdown, DropdownMenu, Spin, Icon, Affix, Select, Option } from 'iview'
import SideTools from '../../home/components/SideTools'
const subscribeList = ['topic_mq/match/*/compareOdds', 'topic_mq/match/*/loOddsData', 'topic_mq/match/*/sportScoreStatus']
export default {
  components: {
    SideTools,
    Dropdown, 
  ...
  methods: {
    ...
    choice (nav, index) {
      let {leagueName, leagueId} = nav
      this.filterTabActive = index
      if(leagueId) {
        let showList = this.allMatch.filter(item => item.leagueId === leagueId).map(item=>{
          return item.matchId
        })
    ...
    getMessage (topic, mqttMsg) {
      ...
      if (mqttMsg.mqttName == 'sportScoreStatus') {
        this.allMatch.filter(Boolean).some(item => {
          if (item.matchId == mqttMsg.matchId) {
            if(mqttMsg.hostTeamScore!=item.hostTeamScore && !this.closeAudio) {
              this.$nextTick(()=>{
      ...
      if (mqttMsg.mqttName === 'loOddsData' && this.bookId == this.bookIdBet365) {
